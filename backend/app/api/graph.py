...
# `from zep_cloud import ApiError` imports a Pydantic model, not an exception
# class — using it in `except` raises TypeError at runtime. The real Zep
# exception lives in zep_cloud.core. We import it tolerantly so deployments
# that drop the zep_cloud dep entirely (postgres-only) keep working.
try:
    from zep_cloud.core import ApiError  # type: ignore
except Exception:  # pragma: no cover - exercised only when zep is uninstalled
    class ApiError(Exception):  # type: ignore
        """Fallback when zep_cloud is unavailable; nothing will raise this."""
...
def _zep_api_user_message(exc: ApiError) -> str:
    """Zep Cloud API 错误转用户可读文案（含用量限额等）。"""
    raw = ""
    if isinstance(exc.body, dict):
        raw = str(exc.body.get("message") or "")
    elif exc.body is not None:
        raw = str(exc.body)
    low = raw.lower()
    if exc.status_code == 403 and "episode usage limit" in low:
        return t("api.zepEpisodeUsageLimit")
    return t("api.zepApiError", status=exc.status_code or 0, message=raw or str(exc))
...
def generate_ontology():
    ...
    try:
        ...
        # R3.4 — when launched from a client workspace, scope the project.
        ...
        # 创建项目
        project = ProjectManager.create_project(name=project_name, client_id=client_id)
        ...
        return jsonify({
            "success": False,
            "error": t(
                'api.requestEntityTooLarge',
                max_mb=Config.MIROFISH_MAX_UPLOAD_MB,
            ),
        }), 413

    except Exception as e:
        logger.error(f"生成本体定义失败: {e}", exc_info=True)
        return jsonify({
            "success": False,
            "error": str(e),
    ...
    try:
        ...
        def build_task():
            ...
            try:
                ...
                user_msg = _zep_api_user_message(e)
                build_logger.error(f"[{task_id}] 图谱构建失败 (Zep API): {user_msg}", exc_info=True)
                project.status = ProjectStatus.FAILED
                project.error = user_msg
                ProjectManager.save_project(project)
                task_manager.update_task(
                    task_id,
                    status=TaskStatus.FAILED,
                    message=t('progress.buildFailed', error=user_msg),
                    error=user_msg,
                )
...
def get_graph_data(graph_id: str):
    ...
    Honors GRAPH_BACKEND: when it is `postgres`, reads from the
    Postgres+pgvector store directly. The Zep branch stays as the legacy
