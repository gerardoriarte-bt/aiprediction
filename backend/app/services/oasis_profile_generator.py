...
class OasisProfileGenerator:
    ...
    def _search_postgres_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Postgres equivalent of the legacy hybrid Zep search.

        Reuses the GraphBackend tools factory (PostgresToolsService) so we
        do not duplicate vector-search SQL here. Output shape stays
        identical to _search_zep_for_entity: {facts, node_summaries, context}.
        """
        results: Dict[str, Any] = {"facts": [], "node_summaries": [], "context": ""}

        if not self.graph_id:
            logger.debug("Skipping postgres search: graph_id is not set")
            return results
        ...
        try:
            ...
            logger.warning(f"Postgres entity search failed ({entity_name}): {e}")
            return results

        # SearchResult.to_dict-equivalent fields: facts, edges, nodes.
        seen_facts = set()
        for fact in (search_result.facts or []):
        ...
        for node in (search_result.nodes or []):
            summary = node.get("summary") if isinstance(node, dict) else None
            other_name = node.get("name") if isinstance(node, dict) else None
            if summary and summary not in seen_summaries:
                seen_summaries.add(summary)
                results["node_summaries"].append(summary)
            if other_name and other_name != entity_name:
                tag = f"Related entity: {other_name}"
                if tag not in seen_summaries:
                    seen_summaries.add(tag)
                    results["node_summaries"].append(tag)
        ...
        logger.info(
            "Postgres entity search complete: %s facts=%d summaries=%d",
            entity_name,
            len(results["facts"]),
